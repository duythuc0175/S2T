...
public class MainCanvas extends JPanel implements Runnable, KeyListener {
    ...
    public static ArrayList<Gundam> tankArray;
    public static ArrayList<Gundam> tankArray2;
    ...
    private Gundam1 playerTank;
    private Gundam2 playerTank2;
    ...
    public MainCanvas() {
        ...
        try {
            ...
            playerTank2 = new Gundam2(t.getPlayerTank2Image(), 32, 32);
            playerTank2.spawnd();
            // player tank is alway at first place of tank array
        ...
        while (!isEndGame()) {
            ...
            if (!isGameOver()) {
                ...
                for (int i = 0; i < tankArray.size(); i++) {
                    Gundam tmp = tankArray.get(i);
                    if (tmp instanceof AIMonster) {
                        ((AIMonster) tmp).think();
                    }

                        Gundam tmp2 = tankArray2.get(i);
                        if (tmp2 instanceof AIMonster) {
                            ((AIMonster) tmp2).think();
                        }

                    tmp2.update();
                    tmp.update();
                    if (tmp instanceof AIMonster && tmp.isDestroyed) {
                        if (((AIMonster) tmp).getType() == AIMonster.RED_TANK) {
                        ...
                        if (MainCanvas.tankArray.size() - 1 < MAX_AITANK_ONSCREEN && MainCanvas.tankArray2.size() - 1 < MAX_AITANK_ONSCREEN) {
                            ...
                            try {
                                ...
                                if (currentTotalAITank % 4 == 0) {
                                    tmp = new AIMonster(t.getRedTankImage(), 32,
                                            32, AIMonster.RED_TANK);
                                } else {
                                    tmp = new AIMonster(t.getAITankImage(), 32,
                                            32, AIMonster.BLUE_TANK);
                                }
    ...
    private void drawRightMenu(Graphics g) {
        ...
        g.drawString(
                String.valueOf(((Gundam1) tankArray.get(0)).getScore()),
                100, 40+20+30);
        g.drawString("HEALTH:", 10, 80+30);
        g.drawString(String
                        .valueOf(((Gundam1) tankArray.get(0)).currentHealth)
                        + "/"
                        + String.valueOf(((Gundam1) tankArray.get(0)).totalHealth),
                100, 80+30);
        g.drawString("HEALTH2:", 10, 80+20+30);
        g.drawString(String
                        .valueOf(((Gundam2) tankArray2.get(0)).currentHealth)
                        + "/"
                        + String.valueOf(((Gundam2) tankArray2.get(0)).totalHealth),
                100, 80+20+30);
        ...
        try {
            BufferedImage lives = MainCanvas.t.getItemImage().getSubimage(64,
                    0, 32, 32);
            for (int i = 0; i < ((Gundam1) tankArray.get(0)).getLives(); i++) {
        ...
        try {
            BufferedImage lives = MainCanvas.t.getItemImage().getSubimage(64,
                    0, 32, 32);
            for (int i = 0; i < ((Gundam2) tankArray2.get(0)).getLives2(); i++) {
